...
def render_fast_pmt_route_picker_map(store_pool, employees_df, route_df=None, show_assigned_layer=True, show_existing_layer=True, component_key="pmt_route_picker"):
    ...
    for _, row in mapped_pool.iterrows():
        ...
        Assigned PMT: {assigned_name}<br>
        Scheduled PMT: {scheduled_name or 'Not active in selected schedule'}<br>
        Use this map as the visual guide for manual route order.
        """
...
def apply_pmt_manage_build_preview(run_id, employee_id, preview_records, notes="", replace_future=False, monthly_target=None):
    ...
    with session_scope("Apply PMT manage schedule preview") as session:
        ...
        for _, row in preview_df.iterrows():
            store_id = int(row["store_id"])
            touched_store_ids.add(store_id)
            proposed_date = pd.to_datetime(row.get("Proposed Date"), errors="coerce")
            if pd.isna(proposed_date):
        ...
        session.flush()
        validation_items = session.scalars(
            select(ScheduleItem)
            .where(
                ScheduleItem.pmt_schedule_run_id == int(run_id),
                ScheduleItem.employee_id == int(employee_id),
    ...
    if runs.empty:
        ...
        if run_items.empty:
            ...
            with map_builder_tab:
                ...
                if st.session_state.get("pmt_map_route_apply_notice"):
                    notice = st.session_state.pop("pmt_map_route_apply_notice")
